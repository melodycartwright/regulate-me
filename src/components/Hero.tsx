...
//10 quotes for rotating on load
const quotes = [
...
//randomly pick a quote on first load
useEffect(() => {
    const randomIndex = Math.floor(Math.random() * quotes.length)
    setQuote(quotes[randomIndex])
}, [])
return (
    <section className='min-h-[70vh] flex flex-col justify-center items-center px-4 sm:px-6'>
        <h1 className='text-xl sm:text-2xl md:text-3xl lg:text-4xl font-medium text-gray-800 animate-fade-in-up'>
            {quote}
        </h1>

        <Link href='/mood' className='mt-6 inline-block px-6 py-3 bg-black text-white rounded-2xl text-sm font-medium transition hover:bg-gray-800'>
        Go to Mood Tracker
        </Link>
    </section>
)
